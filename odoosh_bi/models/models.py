...
class odoosh_bi(models.Model):
    ...
    def _run_process(self, id):
        ...
        try:
            ...
            os.system(f'PGPASSWORD={pg_pass} dropdb --host {db_host} --port "{port}" --username {user_name} --if-exists {db_to_back}')
            # _logger.critical('>>>>>>>>>>>>>>>>>', 3)

            os.system(f'PGPASSWORD={pg_pass} createdb --host {db_host} --port "{port}" --username {user_name} {db_to_back}')
            # _logger.critical('>>>>>>>>>>>>>>>>>', 4)
            ...
            if status == True:
                os.system(f'PGPASSWORD={pg_pass} psql --host {db_host} --port "{port}" --username {user_name} -d {db_to_back} -f {db_back_path}')
                # _logger.critical('>>>>>>>>>>>>>>>>>', 6)
                status = self.checkupdate(val)
                if status == False:
